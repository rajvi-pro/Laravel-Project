...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>Edit Billing</h1>
        <a href="{{ route('admin.billing.show', $billing->id) }}" class="btn btn-secondary">← Back to Details</a>
    </div>
            ...
            <form action="{{ route('admin.billing.update', $billing->id) }}" method="POST">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('staff_id') is-invalid @enderror" id="staff_id" name="staff_id">
                            ...
                            @foreach($staff ?? [] as $staffMember)
                                <option value="{{ $staffMember->id }}" {{ old('staff_id', $billing->staff_id) == $staffMember->id ? 'selected' : '' }}>
                                    {{ $staffMember->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control @error('amount') is-invalid @enderror" 
                                   id="amount" name="amount" value="{{ old('amount', $billing->amount) }}" required>
                        </div>
                        @error('amount')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('payment_status') is-invalid @enderror" id="payment_status" name="payment_status" required>
                            ...
                            <option value="pending" {{ old('payment_status', $billing->payment_status) == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="paid" {{ old('payment_status', $billing->payment_status) == 'paid' ? 'selected' : '' }}>Paid</option>
                            <option value="cancelled" {{ old('payment_status', $billing->payment_status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('payment_method') is-invalid @enderror" id="payment_method" name="payment_method" required>
                            ...
                            <option value="card" {{ old('payment_method', $billing->payment_method) == 'card' ? 'selected' : '' }}>Card</option>
                            <option value="online" {{ old('payment_method', $billing->payment_method) == 'online' ? 'selected' : '' }}>Online</option>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('billing_date') is-invalid @enderror" 
                               id="billing_date" name="billing_date" value="{{ old('billing_date', $billing->billing_date?->format('Y-m-d')) }}" required>
                        @error('billing_date')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('due_date') is-invalid @enderror" 
                               id="due_date" name="due_date" value="{{ old('due_date', $billing->due_date?->format('Y-m-d')) }}">
                        @error('due_date')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" 
                              rows="3">{{ old('notes', $billing->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle"></i> Update Billing
                    </button>
                    <a href="{{ route('admin.billing.show', $billing->id) }}" class="btn btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
